...
telemetry.Errorf("chartHandler: failed to get stats: %v", err)
tmpl, tmplErr := template.ParseFiles("charts.html")
if tmplErr != nil {
telemetry.Errorf("chartHandler: template parse error in error path: %v", tmplErr)
http.Error(w, "Internal Server Error", http.StatusInternalServerError)
return
}
if execErr := tmpl.Execute(w, ChartsData{Months: []MonthStat{}}); execErr != nil {
telemetry.Errorf("chartHandler: template execute error in error path: %v", execErr)
}
return
}
