...
if key == "" {
telemetry.Info("Data is stale but WATTPILOT_KEY not set - skipping auto-refresh")
return false
}

telemetry.Info("Data is stale, attempting automatic refresh...")
err := RefreshData(context.Background())
if err != nil {
telemetry.Errorf("Auto-refresh failed: %v (will use cached/backup data)", err)
return true // Attempted but failed
}

telemetry.Info("Auto-refresh successful")
...
if err != nil {
telemetry.Errorf("Warning: skipping entry with invalid End time %q: %v", entry.End, err)
continue
}
...
if err != nil {
telemetry.Errorf("Warning: failed to create backup for %s: %v", monthKey, err)
...
if err != nil {
telemetry.Fatalf("PrepUrl: failed to parse URL: %v", err)
}
values := myUrl.Query()
if from == "" || to == "" {
values.Del("from")
...
if err != nil {
telemetry.Fatalf("GetUnixTimestampStart: failed to load location: %v", err)
}
...
loc, err := time.LoadLocation("Europe/Berlin")
if err != nil {
telemetry.Fatalf("GetUnixTimestampEnd: failed to load location: %v", err)
}
t, _ := time.Parse("2006-01", yearMonth)
...
if err != nil {
telemetry.Errorf("Main data file unavailable for %s, trying backup: %v", monthToCalculate, err)
...
jsonData = backupData
telemetry.Infof("Using backup data for month %s", monthToCalculate)
}
...
if usedMainFile {
telemetry.Errorf("Failed to parse main JSON: %v, trying backup", err)
backupData, backupErr := tryMonthlyBackup(monthToCalculate)
if backupErr != nil {
...
telemetry.Infof("Using backup data for month %s after parse failure", monthToCalculate)
} else {
return WattpilotData{}, fmt.Errorf("failed to parse JSON: %v", err)
}
}
...
_, span := telemetry.StartSpan(ctx, "RefreshData")
defer span.End()
...
telemetry.Errorf("Warning: failed to create some backups: %v", err)
} else {
telemetry.Info("Successfully created monthly backups")
}

telemetry.Info("Data refreshed successfully")
return nil
}
